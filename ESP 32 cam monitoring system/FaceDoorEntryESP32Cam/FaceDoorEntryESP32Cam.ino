...
void open_door(WebsocketsClient &client) {
  if (digitalRead(relay_pin) == LOW) {
    ...
    Serial.println("Known Person Detected");
      ...
      if (out_res.net_boxes)
      {
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK)
        {
          ...
          if (g_state == START_DETECT) {
            client.send("Known Face detected");
          }
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            if (f)
            {
              ...
              sprintf(recognised_message, "Detected Person Name : %s", f->id_name);
              open_door(client);
              client.send(recognised_message);
            }
            else
            {
              client.send("Unknown Face Detected in 1st Floor\nDrown has been deployed");
              Serial.println("Unknown Person Detected in 1st Floor");
              Serial.println("Lattitude : 11.679230, Longitude : 78.125179 of the Back gate");
            }
          }
